...
function head({ title, description, file, type = 'website' }) {
  ...
  const graph = [
      ...
      },
      {
        ...
        primaryImageOfPage: `${siteUrl}/assets/icon.png`
      }
  ];
...
<head>
  ...
  <meta property="og:image" content="${siteUrl}/assets/icon.png">
  ...
  <meta name="twitter:image" content="${siteUrl}/assets/icon.png">
...
const warningBody = `${hero({ eyebrow: 'درس الفئة الأولى', title: 'إشارات التحذير', text: 'هذه الإشارات لا تمنعك مباشرة، لكنها تنبهك إلى خطر أو تغير محتمل حتى تمنح نفسك وقتاً ومسافة آمنة للتصرف.' })}<section class="section section--white"><div class="container signal-layout"><div class="signal-grid" data-source="warning-signs"></div><aside class="info-panel"><h2>قاعدة التحذير</h2><p>عندما ترى مثلث تحذير، لا تنتظر حتى تصبح قريباً من الخطر. خفف السرعة واستعد لتغيير سلوكك بهدوء.</p><a class="button" href="quiz-warning.html">اختبر إشارات التحذير</a></aside></div></section>`;
...
const prohibitionBody = `${hero({ eyebrow: 'درس الفئة الثانية', title: 'إشارات المنع والتنظيم', text: 'تخبرك هذه الفئة بما لا يُسمح به أو تحدد لك قيداً تنظيمياً يجب احترامه في المقطع الذي تبدأ عنده الإشارة.' })}<section class="section section--white"><div class="container signal-layout"><div class="signal-grid" data-source="prohibition-signs"></div><aside class="info-panel"><h2>قاعدة المنع</h2><p>إشارة المنع واضحة في رسالتها: لا تتجاوز القيد حتى لو بدا الطريق خالياً. اختر مساراً قانونياً أو انتظر نهاية المنع.</p><a class="button" href="quiz-prohibition.html">اختبر إشارات المنع</a></aside></div></section>`;
...
const mandatoryBody = `${hero({ eyebrow: 'درس الفئة الثالثة', title: 'إشارات الإلزام', text: 'تحدد هذه الإشارات الاتجاه أو السلوك الواجب اتباعه. اقرأ السهم مبكراً، واختر المسار قبل أن تصل إلى التقاطع.' })}<section class="section section--white"><div class="container signal-layout"><div class="signal-grid" data-source="mandatory-signs"></div><aside class="info-panel"><h2>قاعدة الإلزام</h2><p>اتباع الاتجاه لا يعفيك من المراقبة. افحص المرايا، راقب المشاة، ثم نفّذ الحركة بهدوء وفي المسار الصحيح.</p><a class="button" href="quiz-mandatory.html">اختبر إشارات الإلزام</a></aside></div></section>`;
...
const informationBody = `${hero({ eyebrow: 'درس الفئة الرابعة', title: 'الإشارات الإرشادية', text: 'تساعدك هذه اللوحات على فهم الوجهات والخدمات واختيار مسارك مبكراً من دون ارتباك أو تغيير مفاجئ للمسار.' })}<section class="section section--white"><div class="container signal-layout"><div class="signal-grid" data-source="information-signs"></div><aside class="info-panel"><h2>قاعدة الإرشاد</h2><p>اللوحة الإرشادية تمنحك وقتاً للتخطيط. عندما تراها مبكراً، اختر المسار المناسب بهدوء بدلاً من مناورات اللحظة الأخيرة.</p><a class="button" href="quiz-information.html">اختبر الإشارات الإرشادية</a></aside></div></section>`;
